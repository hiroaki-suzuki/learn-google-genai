...
- [x] `.env`ファイルに本番用の品質スコア閾値を定義する（例：QUALITY_SCORE_THRESHOLD=0.8）
- [x] `.env.test`ファイルにテスト用の品質スコア閾値を定義する（例：QUALITY_SCORE_THRESHOLD=0.5）
- [x] `.env.example`ファイルに設定例とコメントを記載する
- [x] 各ファイルで設定する環境変数名が統一されている
...
- [x] ENV環境変数が"test"の場合、`.env.test`を読み込む
- [x] ENV環境変数が未設定または"production"の場合、`.env`を読み込む
- [x] 環境変数ファイルが存在しない場合、適切なエラーメッセージを表示する
- [x] 環境変数の読み込み処理を専用の関数またはモジュールに実装する
- [x] 品質チェック（フォーマット、Lint、型チェック）が通過すること
- [x] テストがすべて合格すること
...
- [x] 品質評価ロジック内のハードコードされた閾値を環境変数から取得するよう変更する
- [x] 環境変数が未設定の場合のデフォルト値を設定する（例：0.8）
- [x] 環境変数の値が不正な場合（数値以外、範囲外など）の例外処理を実装する
- [x] 変更後もロジックの動作が既存と同じであることを確認する
- [x] 品質チェック（フォーマット、Lint、型チェック）が通過すること
- [x] テストがすべて合格すること
- [x] テストカバレッジが基準値を満たすこと
...
- [x] テスト実行時にENV=testが設定されるよう、テスト設定ファイル（pytest.ini等）を更新する
- [x] 既存のテストがすべて.env.testの閾値で正常に動作することを確認する
- [x] テスト実行時に.envではなく.env.testが読み込まれていることをログまたはアサーションで確認する
- [x] テストコマンド実行時の環境変数設定方法をREADMEまたはドキュメントに記載する
- [x] 品質チェック（フォーマット、Lint、型チェック）が通過すること
- [x] テストがすべて合格すること
- [x] コンソール出力で.env.testが正しく読み込まれていることを確認する
