...
- [ ] `.env.example`内の`QUALITY_SCORE_THRESHOLD`を`4.5`に設定する
- [ ] スコア閾値の有効範囲が0.0〜5.0であることをコメントで明記する
- [ ] 設定例として「テスト環境: 3.5、本番環境: 4.5」をコメントで追加する
...
- [ ] `app/.env`ファイルの`QUALITY_SCORE_THRESHOLD`を`4.5`に設定する
- [ ] ファイルがGit管理対象外であることを確認する（`git status`で表示されない）
- [ ] コンソール出力で変更が正しく反映されていることを確認する
...
- [ ] `cd app && uv run pytest`でテストが全て通過することを確認する
- [ ] テスト実行時に`ENV=test`が自動設定され、`.env.test`が読み込まれることを確認する
- [ ] テスト環境でのスコア閾値が3.5として機能することを確認する
- [ ] 本番環境（`uv run main.py`）でのスコア閾値が4.5として機能することを確認する
- [ ] 品質チェック（フォーマット、Lint、型チェック）が通過すること
- [ ] テストがすべて合格すること
- [ ] テストカバレッジが基準値を満たすこと
- [ ] コンソール出力で正しいスコア閾値が使用されていることを確認する
