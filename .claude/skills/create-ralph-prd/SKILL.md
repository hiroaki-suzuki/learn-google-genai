...
Ralphは反復ごとに新しいインスタンスを生成し、以前の作業内容を記憶しません。もしストーリーが大きすぎると、AIは作業途中でコンテキストを失い、不完全なコードを生成することになります。
...
各基準は、Ralphが実際に確認できる具体的な内容でなければなりません。曖昧な表現は厳禁です。
...
```
「品質チェック（フォーマット、Lint、型チェックが通過すること」
「テストが通過すること」
```

### ファイル出力やログ出力を伴うストーリーの場合は、以下も追加してください：

```
「コンソールの出力、実ファイルで変更が正しく反映されていることを確認する」
```
...
- [ ] 各ストーリーに「品質チェック通過」「テスト合格」「カバレッジ網羅率」を検証基準として設定しているか
- [ ] ファイル出力などのストーリーには「コンソールで変更が正しく動作することを確認」を検証基準として設定しているか
